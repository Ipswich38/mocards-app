-- =============================================================================
-- PRODUCTION READY SCHEMA V3.0 - FINAL CORRECTED VERSION
-- =============================================================================
-- Complete schema with all fixes applied
-- FIXED: Table name typo and column reference errors
-- TESTED: All SQL syntax validated
...
-- Default perk templates (CORRECTED TABLE NAME)
...
-- =============================================================================
-- 8. SAMPLE DATA (Default Perks) - CORRECTED
-- =============================================================================
...
-- =============================================================================
-- 9. UPDATED FUNCTIONS FOR V3.0 - CORRECTED
-- =============================================================================
...
-- =============================================================================
-- 10. ESSENTIAL MIGRATIONS FOR EXISTING DATABASE
-- =============================================================================

-- Fix passcode constraint if it exists
DO $$
BEGIN
  -- Make passcode nullable
  BEGIN
    ...
    RAISE NOTICE 'Passcode constraint fixed successfully';
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE 'Passcode constraint already fixed or table structure different';
  END;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE 'Columns already exist or other issue: %', SQLERRM;
  END;

  -- Add redemption tracking to card_perks if missing
  BEGIN
    ALTER TABLE public.card_perks ADD COLUMN IF NOT EXISTS claimed_by VARCHAR(255);
    ALTER TABLE public.card_perks ADD COLUMN IF NOT EXISTS claimed_at_clinic_id UUID REFERENCES public.mocards_clinics(id);
    ALTER TABLE public.card_perks ADD COLUMN IF NOT EXISTS redemption_notes TEXT;
    ALTER TABLE public.card_perks ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW();
    RAISE NOTICE 'Card perks redemption fields added successfully';
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE 'Card perks already has redemption fields or other issue: %', SQLERRM;
  END;

  -- Migrate any existing default_perk_datalates to default_perk_templates
  BEGIN
    INSERT INTO public.default_perk_templates (perk_name, perk_type, perk_value, description, display_order, created_at)
    SELECT perk_name, perk_type, perk_value, description, display_order, created_at
    FROM public.default_perk_datalates
    ON CONFLICT (perk_type) DO NOTHING;

    RAISE NOTICE 'Migrated data from default_perk_datalates to default_perk_templates';
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE 'No default_perk_datalates table to migrate or already migrated';
  END;
...
SELECT
  ...
  CASE
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'default_perk_templates')
    THEN '✅ Perk templates table ready'
    ELSE '❌ Missing perk templates table'
  END;
...
SELECT
  ...
  (SELECT COUNT(*) FROM public.perk_redemptions) as total_redemptions,
  (SELECT COUNT(*) FROM public.default_perk_templates WHERE is_active = true) as active_perk_templates;