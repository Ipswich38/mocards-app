-- ===================================================================
-- MOCARDS PRODUCTION SCHEMA UPGRADE
-- Enhances existing schema for full clinic management capabilities
-- ===================================================================

-- Add new columns to existing mocards_clinics table for production features
...
-- Create function to generate clinic code
...
BEGIN
    LOOP
        ...
        new_code := 'CL' || UPPER(SUBSTRING(MD5(RANDOM()::TEXT) FROM 1 FOR 4));

        -- Check if code already exists
        SELECT EXISTS(SELECT 1 FROM public.mocards_clinics WHERE clinic_code = new_code) INTO code_exists;
...
-- Create function to generate secure clinic password
CREATE OR REPLACE FUNCTION generate_clinic_password()
RETURNS TEXT AS $$
DECLARE
...
-- Create function to update monthly card count
CREATE OR REPLACE FUNCTION update_monthly_card_count()
RETURNS TRIGGER AS $$
BEGIN
    -- Reset monthly count if it's a new month
    IF EXTRACT(MONTH FROM NOW()) != EXTRACT(MONTH FROM (
        SELECT last_login_at FROM public.mocards_clinics
        WHERE id = NEW.assigned_clinic_id
    )) THEN
        UPDATE public.mocards_clinics
        SET cards_issued_this_month = 1,
            last_login_at = NOW()
        WHERE id = NEW.assigned_clinic_id;
    ELSE
        -- Increment monthly count
        UPDATE public.mocards_clinics
        SET cards_issued_this_month = cards_issued_this_month + 1
        WHERE id = NEW.assigned_clinic_id;
    END IF;
...
-- Create trigger to track monthly card activation
CREATE TRIGGER trigger_update_monthly_card_count
    AFTER UPDATE ON public.cards
    FOR EACH ROW
    WHEN (OLD.status = 'unactivated' AND NEW.status = 'activated')
    EXECUTE FUNCTION update_monthly_card_count();

-- Verification queries
SELECT 'Production schema upgrade completed!' as status;

SELECT
    'Enhanced Clinic Table' as info,
...
SELECT
    ...
    monthly_card_limit,
    price_per_month
FROM public.clinic_subscription_plans
...
-- - Sales tracking with commissions
-- - Perk redemption analytics
-- - Monthly reporting system
-- - Automated clinic code/password generation
-- - Monthly card limit enforcement
-- ===================================================================