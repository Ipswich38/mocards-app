-- ===================================================================
-- MOCARDS APPOINTMENT BOOKING SYSTEM SCHEMA (FINAL COMPATIBLE VERSION)
-- Handles existing table structure and missing columns
-- ===================================================================

-- First, let's check what columns exist in the cards table and add missing ones
DO $$
BEGIN
  -- Add card_id column to cards table if it doesn't exist (self-referencing for compatibility)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'cards' AND column_name = 'card_id'
  ) THEN
    ALTER TABLE public.cards ADD COLUMN card_id UUID DEFAULT gen_random_uuid();
    -- Update existing records
    UPDATE public.cards SET card_id = id WHERE card_id IS NULL;
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.appointments (
  ...
  -- Card and Patient Information (use id instead of card_id)
...
BEGIN
  ...
  -- Ensure card_id column exists in appointments
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'appointments' AND column_name = 'card_id'
  ) THEN
    ALTER TABLE public.appointments ADD COLUMN card_id UUID REFERENCES public.cards(id) ON DELETE CASCADE;
  END IF;
END $$;
...
-- VERIFICATION
-- ===================================================================
SELECT 'Appointment booking schema setup completed successfully!' as status;