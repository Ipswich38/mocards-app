-- Enhanced Production Schema for Location-Based Passcode System
...
CREATE OR REPLACE FUNCTION generate_control_number(batch_prefix TEXT, position INTEGER)
RETURNS TEXT AS $$
BEGIN
    RETURN batch_prefix || '-' || LPAD(position::TEXT, 3, '0');
END;
$$ LANGUAGE plpgsql;
...
ALTER TABLE cards
ADD CONSTRAINT valid_location_code CHECK (LENGTH(location_code) = 3);
