-- Safe Production Schema for Location-Based Passcode System
...
-- Fix existing location_code data to meet 3-character constraint
UPDATE cards
SET location_code = 'PHL'
WHERE location_code IS NULL OR LENGTH(location_code) != 3;
...
-- Add RLS policies for enhanced card system (safe method - drop existing first)
ALTER TABLE location_code_assignments ENABLE ROW LEVEL SECURITY;

-- Safely drop existing policies if they exist
DO $$
BEGIN
    DROP POLICY IF EXISTS "Clinics can manage their location assignments" ON location_code_assignments;
    DROP POLICY IF EXISTS "Admin can manage all location assignments" ON location_code_assignments;
EXCEPTION
...
CREATE POLICY "Admin can manage all location assignments" ON location_code_assignments
    FOR ALL USING (
        ...
        auth.uid() IN (SELECT id FROM mocards_admin_users)
    );

-- Add constraints to ensure data integrity (safe method - after fixing data)
DO $$
BEGIN
    ALTER TABLE cards DROP CONSTRAINT IF EXISTS valid_location_code;
    ALTER TABLE cards ADD CONSTRAINT valid_location_code CHECK (LENGTH(location_code) = 3);
EXCEPTION
    WHEN check_violation THEN
        -- If there are still violations, show which records are problematic
        RAISE NOTICE 'Some cards still have invalid location codes. Run: SELECT id, location_code FROM cards WHERE LENGTH(location_code) != 3;';
END $$;

-- Create view for enhanced card information (drop existing first)
DROP VIEW IF EXISTS enhanced_card_view;
CREATE VIEW enhanced_card_view AS
SELECT
    c.*,
    cb.batch_number,
    cb.batch_metadata,
...
-- Update existing card counts for all batches
UPDATE card_batches
SET cards_generated = (
    SELECT COUNT(*)
    FROM cards
    WHERE cards.batch_id = card_batches.id
);

-- Verification message
SELECT 'Production schema setup completed successfully!' as status;