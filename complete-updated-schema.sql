-- ===================================================================
-- MOCARDS COMPLETE UPDATED SCHEMA
-- Comprehensive schema including IT monitoring and enhanced perk management
-- ===================================================================
...
CREATE TABLE IF NOT EXISTS public.it_system_metrics (
  ...
  metric_unit VARCHAR(20), -- %, MB, ms, req/s, etc.
  metric_category VARCHAR(50), -- 'performance', 'resource', 'availability'
...
CREATE TABLE IF NOT EXISTS public.it_database_performance (
  ...
  user_context JSONB, -- admin_id, clinic_id, etc.
...
CREATE TABLE IF NOT EXISTS public.it_error_tracking (
  ...
  user_context JSONB, -- user info when error occurred
  request_context JSONB, -- request details
  environment VARCHAR(20) DEFAULT 'production',
...
-- Application feature usage analytics
CREATE TABLE IF NOT EXISTS public.it_feature_usage (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  timestamp TIMESTAMP DEFAULT NOW(),
  feature_name VARCHAR(100) NOT NULL,
  feature_category VARCHAR(50), -- 'admin', 'clinic', 'cardholder'
...
-- IT monitoring policies (restrictive - only for IT admins)
CREATE POLICY "IT admin only access" ON public.it_admin_accounts FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
CREATE POLICY "IT admin logs access" ON public.it_activity_logs FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
CREATE POLICY "IT admin metrics access" ON public.it_system_metrics FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
CREATE POLICY "IT admin db performance access" ON public.it_database_performance FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
CREATE POLICY "IT admin security access" ON public.it_security_events FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
CREATE POLICY "IT admin error access" ON public.it_error_tracking FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
CREATE POLICY "IT admin feature usage access" ON public.it_feature_usage FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
CREATE POLICY "IT admin api monitoring access" ON public.it_api_monitoring FOR ALL USING (auth.uid() IN (SELECT id FROM public.it_admin_accounts WHERE is_active = true));
...
-- IT monitoring permissions (service role only)
GRANT ALL ON public.it_admin_accounts TO service_role;
GRANT ALL ON public.it_activity_logs TO service_role;
GRANT ALL ON public.it_system_metrics TO service_role;
GRANT ALL ON public.it_database_performance TO service_role;
GRANT ALL ON public.it_security_events TO service_role;
GRANT ALL ON public.it_error_tracking TO service_role;
GRANT ALL ON public.it_feature_usage TO service_role;
GRANT ALL ON public.it_api_monitoring TO service_role;

-- ===================================================================
-- TRIGGER FUNCTIONS FOR PERK MIRRORING
-- ===================================================================
...
-- ===================================================================
-- IT MONITORING TRIGGER FUNCTIONS
-- ===================================================================

-- Function to log all database operations for audit
...
-- Create database operation triggers for key tables
CREATE OR REPLACE TRIGGER trigger_log_perk_template_operations
    AFTER INSERT OR UPDATE OR DELETE ON public.perk_templates
    FOR EACH ROW
    EXECUTE FUNCTION public.log_database_operation();

CREATE OR REPLACE TRIGGER trigger_log_clinic_customization_operations
    AFTER INSERT OR UPDATE OR DELETE ON public.clinic_perk_customizations
    FOR EACH ROW
    EXECUTE FUNCTION public.log_database_operation();
...
/*
This complete updated schema includes:

1. ENHANCED PERK MANAGEMENT SYSTEM:
   ...
   - perk_usage_analytics (usage tracking and reporting)
   - Automatic mirroring system with triggers

2. IT MONITORING & AUDIT SYSTEM:
   - it_admin_accounts (IT staff accounts)
   - it_activity_logs (comprehensive activity logging)
   - it_system_metrics (real-time system monitoring)
   - it_database_performance (database performance tracking)
   - it_security_events (security events and audit trail)
   - it_error_tracking (error tracking and debugging)
   - it_feature_usage (application feature analytics)
   - it_api_monitoring (API endpoint monitoring)
...
The schema provides:
✅ Full CRUD operations for admin perk management
✅ Automatic perk mirroring to all clinics
✅ Complete clinic customization capabilities
✅ Comprehensive IT monitoring and troubleshooting
