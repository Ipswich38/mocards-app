...
ADD COLUMN IF NOT EXISTS assigned_clinic_id UUID REFERENCES public.mocards_clinics(id);
...
CREATE TABLE IF NOT EXISTS public.card_assignment_history (
    ...
    clinic_id UUID REFERENCES public.mocards_clinics(id) ON DELETE SET NULL,
    assignment_type VARCHAR(50) NOT NULL, -- 'assigned', 'unassigned', 'transferred'
    assigned_by_type VARCHAR(50) NOT NULL, -- 'admin', 'clinic', 'system'
    assigned_by_id VARCHAR(255) NOT NULL,
    assigned_by_name VARCHAR(255),
    previous_clinic_id UUID REFERENCES public.mocards_clinics(id) ON DELETE SET NULL,
    assignment_reason TEXT,
    assignment_details JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT valid_assignment_type CHECK (assignment_type IN ('assigned', 'unassigned', 'transferred')),
    CONSTRAINT valid_assigned_by_type CHECK (assigned_by_type IN ('admin', 'clinic', 'system'))
);
