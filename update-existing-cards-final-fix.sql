-- Update Existing Cards to Proper V2 Format - FINAL FIX
-- Handles materialized view dependencies properly
-- SAFE EXECUTION - Drops and recreates dependent views safely
...
CREATE TABLE IF NOT EXISTS cards_backup_before_final_format_update AS
...
FROM pg_matviews
WHERE matviewname = 'search_optimized_cards'
LIMIT 1;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'system_audit_log') THEN
        ...
        VALUES (
            'CARDS_FORMAT_FINAL_UPDATE',
            'Successfully updated cards to proper V2 format with materialized view handling',
            jsonb_build_object(
...
SELECT
    '🎉 CARD FORMAT UPDATE COMPLETED SUCCESSFULLY!' as result;

SELECT
    '✅ Column dependencies resolved' as technical_1,
