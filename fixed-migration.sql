-- ===================================================================
-- MOCARDS FIXED MIGRATION SCRIPT
-- Handles the clinic_code column issue properly
-- ===================================================================
...
-- Update existing clinics without clinic_code
UPDATE clinics
SET clinic_code = 'CLINIC' || LPAD(ROW_NUMBER() OVER (ORDER BY id)::text, 3, '0')
WHERE clinic_code IS NULL;

-- Make clinic_code NOT NULL and UNIQUE after populating it
...
-- Insert demo clinic only if it doesn't exist
...
-- Show clinics (with their new clinic_codes)
SELECT 'Clinics:' as info, clinic_code, clinic_name FROM clinics LIMIT 5;
