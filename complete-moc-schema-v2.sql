-- MOC Card System V2.0 Schema (COMPLETE & CONFLICT-FREE)
-- Complete database schema for the new MOC system
-- This version handles all potential conflicts from previous runs
...
-- 4. Handle default perks templates table properly
-- Drop existing table and related policies first
DROP POLICY IF EXISTS "Allow public read perk templates" ON default_perk_templates;
DROP POLICY IF EXISTS "Allow admin all operations perk templates" ON default_perk_templates;
DROP TRIGGER IF EXISTS update_perk_templates_updated_at ON default_perk_templates;
DROP TABLE IF EXISTS default_perk_templates CASCADE;

-- Create the table with all required columns
CREATE TABLE default_perk_templates (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
ALTER TABLE card_perks ADD COLUMN IF NOT EXISTS perk_template_id UUID;
ALTER TABLE card_perks ADD COLUMN IF NOT EXISTS auto_assigned BOOLEAN DEFAULT false;

-- Add foreign key constraint if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'card_perks_perk_template_id_fkey') THEN
        ALTER TABLE card_perks ADD CONSTRAINT card_perks_perk_template_id_fkey
        FOREIGN KEY (perk_template_id) REFERENCES default_perk_templates(id);
    END IF;
END $$;
...
('01', 'National Capital Region (NCR)', 'Metro Manila and surrounding areas'),
('02', 'Cordillera Administrative Region (CAR)', 'Mountain provinces'),
...
-- 12. Enable Row Level Security (RLS) and create policies
...
DROP POLICY IF EXISTS "Allow public read location codes v2" ON location_codes_v2;
DROP POLICY IF EXISTS "Allow public read clinic codes" ON clinic_codes_by_region;
DROP POLICY IF EXISTS "Allow public read perk templates" ON default_perk_templates;
DROP POLICY IF EXISTS "Allow public read activation log" ON card_activation_log;

DROP POLICY IF EXISTS "Allow admin all operations location codes v2" ON location_codes_v2;
DROP POLICY IF EXISTS "Allow admin all operations clinic codes" ON clinic_codes_by_region;
DROP POLICY IF EXISTS "Allow admin all operations perk templates" ON default_perk_templates;
DROP POLICY IF EXISTS "Allow admin all operations activation log" ON card_activation_log;
...
DROP TRIGGER IF EXISTS update_location_codes_v2_updated_at ON location_codes_v2;
DROP TRIGGER IF EXISTS update_clinic_codes_updated_at ON clinic_codes_by_region;
DROP TRIGGER IF EXISTS update_perk_templates_updated_at ON default_perk_templates;
